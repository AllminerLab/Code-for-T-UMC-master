...
if ~exist('mode','var')
    ...
    % mode = 2 --> front slice
    % mode = 3 --> top slice X(i::)
    mode = 1;
end

X=reshape(x,sX); % 将X转化为N*N*V
if mode == 1
    Y=X2Yi(X,3);
elseif mode == 3
    Y=shiftdim(X, 1); %X从NNK -> NKN 向左移动一位
else
    Y = X;
end
...
if isinteger(n3/2)
    ...
    for i = 1:endValue
        ...
        if isWeight
            ...
            tau = rho;
%             tau = (sum(shat)/shat)/rho;
            shat = max(shat - tau,0); % 对每个view 内的特征值进行不同程度的shrink
           
        end
        objV = objV + sum(shat(:));
        Yhat(:,:,i) = uhat*shat*vhat';
        if i > 1 
            Yhat(:,:,n3-i+2) = conj(uhat)*shat*conj(vhat)'; %希望对每个view进行不同程度shirink（一开始参数都为1，然后根据特征值shat总大小进行shrink？）
            objV = objV + sum(shat(:));
        end
    end
end
